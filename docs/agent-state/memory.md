...
## V1.5A decisions (OpenAPI import + Creator Test Console)

- **Parse core is pure, injectable, network-free.** `lib/openapi/parse.ts` never fetches or executes anything: bound size (1 MiB) → JSON/YAML parse (data-only; the `yaml` library never executes tags) → OpenAPI version gate (3.0.x/3.1.x only; Swagger rejected) → external `$ref` scan (any http(s) `$ref` fails fast — a single self-contained file only) → validation (injectable; the default `@readme/openapi-parser` runs with `resolve.external: false`, so it can never fetch remote schemas) → operation discovery. Every failure maps to a machine error code (`not_openapi` / `invalid_syntax` / `unsupported_version` / `too_large` / `validation_failed`); raw parser internals are never surfaced.
- **Server precedence follows the spec**: operation `servers` → path-level `servers` → root `servers` (first entry wins at each level). Server URL template variables are substituted from their `default`s; a variable without a default makes the server unusable (warning, treated as unset). Relative server URLs are treated as unset — import requires an absolute base URL.
- **Split-route model — NO schema change.** Each imported operation publishes a normal `proxy_routes` row with `upstreamUrl` = effective server base + the operation path. Operations with path params keep the operation path as `callerPathTemplate` — displayed as the documented caller path appended to the powered URL (callers substitute values; the gateway forwards the segments after the slug). No new route fields, no migration, no `drizzle` changes. This is the key architecture decision: import reuses the existing upstream-url + caller-path-segments composition.
- **Publish idempotency limitation (accepted for V1.5A).** `POST /api/openapi/publish` has NO server idempotency key: two identical publishes create two routes. The client mitigates with an in-flight guard and retry-failed-only semantics (only operations that failed are retried; successful results are retained by key). Documented in the route and this is a known limitation, not a bug — a server-side idempotency key is deferred.
- **Transient test secrets are never persisted.** Draft-mode test requests (`POST /api/endpoints/test`) take the submitted secret through the REAL publish path machinery — `validateUpstreamAuth`, then AES-256-GCM encryption → the service decrypts it → the M11.1 `onDecrypt` wiring auto-registers the plaintext with the log redactor. Nothing is stored, nothing is logged by the console module. Stored-credential mode decrypts server-side only and never discloses the value to the caller.
- **Preview redaction for echo upstreams.** A test against httpbin/postman-echo can reflect the injected credential in the response body; `lib/testconsole/core.ts` redacts the preview server-side with the active secret's exact value before pretty-printing/truncation — the secret is never shown, by construction. Previews are capped at 64 KiB (backend capture cap stays 5 MiB).
- **The test console is the same hardened path, not a parallel client.** It executes through the production `upstreamService.executeUpstream` (runtime SSRF revalidation + DNS pin, decrypt, header filtering + injection after, compression normalization, 1 MiB in / 5 MiB out / 30 s, redirects never followed) with a fixed informational route identity. There is no payment path: no x402 challenge, no /verify, no /settle, no ledger entry, no payout, no signer, no blockchain activity — the only side effect is the upstream request itself. Money-safety statement stands: the test console is plain HTTPS GET/POST through the SSRF-safe service with zero financial activity.
- **Three new rate-limit surfaces (V1.5A), same semantics as M11** — per client IP, session-authenticated, fail-open degraded, bounded TTL, same 429 shape: `openapiParse` 10/60, `openapiPublish` 10/60, `endpointTest` 20/60 (`lib/ratelimit/policy.ts` now has six surfaces).
- **Batch publish is sequential and partial-failure.** Operations are created one at a time (no parallel creates → no slug races); each is isolated so one failure never aborts the batch; the response is always 200 + `{results}` with per-operation machine codes from `toEndpointErrorResponse`. Identity is always the authenticated session — never from the body.
